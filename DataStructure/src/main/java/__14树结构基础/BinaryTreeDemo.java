...
class BinaryTree {
    ...
    public HeroNode postOrderSearch(int no) {
        if(root != null) return this.root.postOrderSearch(no);
        else return null;
    }
...
class HeroNode {
    ...
    //编写前序遍历的方法
    public void preOrder() {
        System.out.println(this); //先输出父结点或者当前节点
        //递归向左子树前序遍历
    ...
    public HeroNode infixOrderSearch(int no) {
        HeroNode resNode = null;
        //判断当前结点的左子节点是否为空，如果不为空，则递归中序查找(有可能在最后的左节点才能找到,因此必须要自上而下遍历完一遍,然后才能把自下而上的结果拿回来)
