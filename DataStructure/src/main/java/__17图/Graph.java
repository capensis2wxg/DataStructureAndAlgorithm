...
public class Graph {
    ...
    public static void main(String[] args) {
        ...
        //循环的添加顶点
        for(String vertex: Vertexes) graph.insertVertex(vertex);
        //添加边
        //A-B A-C B-C B-D B-E
        ...
        //显示一把邻结矩阵
        graph.showGraph();
        System.out.println(graph.getNextNeighbor(1, 3));
...
//		System.out.println();
    ...
    /**
     * 得到某个节点的第一个邻接结点的下标 w
     * @param index 第几个节点（A， B， C， D， E）
     * @return 如果存在就返回对应的下标，否则返回-1
     */
    public int getFirstNeighbor(int index) {
        for(int j = 0; j < vertexList.size(); j++) if(edges[index][j] > 0) return j;
        return -1;
    }

    //根据前一个邻接结点的下标来获取下一个邻接结点
    public int getNextNeighbor(int v1, int v2) {
        for(int j = v2 + 1; j < vertexList.size(); j++) if(edges[v1][j] > 0) return j;
        return -1;
    }
